    ...
    render() {
        ...
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2, mb: 2 }}>
                <TextField
                    select
                    label="Resource spending order"
                    value={this.state.rp_idx}
                    onChange={e => this.handleChange(e, 'rp_idx')}

                    fullWidth
                    size="small"
                >
                    {resource_priorities.map((prio, idx) => (
                        <MenuItem value={idx} key={idx}>
                            {resource_names[prio[0]] + '>' + resource_names[prio[1]] + '>' + resource_names[prio[2]]}
                        </MenuItem>
                    ))}
                </TextField>
                <TextField
                    select
                    label="Spare resource policy"
                    value={this.state.spare_policy}
                    onChange={e => this.handleChange(e, 'spare_policy')}

                    fullWidth
                    size="small"
                >
                    {policy_names.map((policy, idx) => (
                        <MenuItem value={idx} key={policy}>{policy}</MenuItem>
                    ))}
                </TextField>
            </Box>
        );
