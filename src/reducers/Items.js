    ...
    switch (action.type) {
        ...
        case SETTINGS: {
            if (action.payload.statname === 'accslots') {
                ...
                const currentCount = state.equip.accessory.length;
                if (targetCount === currentCount) {
                    return state;
                }

                let accessory = [...state.equip.accessory];
                if (targetCount > currentCount) {
                    // Add empty slots
                    for (let i = 0; i < targetCount - currentCount; i++) {
                        accessory.push(new EmptySlot(Slot.ACCESSORY).id);
                    }
                } else {
                    // Remove slots
                    accessory = accessory.slice(0, targetCount);
                }
